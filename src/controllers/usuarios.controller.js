...
export const crearUsuarioPostulante = async (req, res) => {
  ...
  try {
    ...
    let archivo_cv, certificado_unico_laboral;
    if (req.files?.archivo_cv) {
      const result = await uploadImage(req.files.archivo_cv.tempFilePath);
      archivo_cv = result.secure_url;
    }

    if (req.files?.certificado_unico_laboral) {
      const result = await uploadImage(
        req.files.certificado_unico_laboral.tempFilePath
      );
      certificado_unico_laboral = result.secure_url;
    }
